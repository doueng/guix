...
2. **Passwords are managed with `passwd(1)`.** `make-ssd-os` leaves account password fields unset (`#f`), which follows the Guix manual and preserves passwords set with `passwd` across reconfigure without placing hashes in the store. A fresh installation must set account passwords before use. The USB root is unencrypted: it is unsuitable for sensitive data without accepting physical-access exposure.
