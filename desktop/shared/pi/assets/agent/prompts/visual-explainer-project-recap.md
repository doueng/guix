...
- Shorthand like `2w`, `30d`, `3m`: parse to a `jj` revset date filter (`2w` → `author_date(after:"2 weeks ago")`, `30d` → `author_date(after:"30 days ago")`, `3m` → `author_date(after:"3 months ago")`)
...
2. **Recent activity.** `jj log -r 'author_date(after:"<window>")'` for recent change history. `jj log -r 'author_date(after:"<window>")' --stat` for file-level change scope. For contributor activity, summarize authors from that same revision set (for example with `jj log ... --no-graph -T 'author.name() ++ "\n"' | sort | uniq -c | sort -nr`). Identify which areas of the codebase were most active.

3. **Current state.** Check for working-copy changes (`jj status`). Check for stale or divergent bookmarks (`jj bookmark list`). Look for TODO/FIXME comments in recently changed files. Read progress docs if they exist (`~/.agent/memory/{project}/progress.md`, `~/.pi/agent/memory/{project}/progress.md`, `.pi/todos/`, or similar).

4. **Decision context.** Read recent change descriptions and `jj show` output for rationale. If running in the same session as recent work, mine the conversation history. Read any plan docs, RFCs, or ADRs in the project directory.
...
- Every quantitative figure: change counts, file counts, line counts, bookmark counts
- Every module, function, and type name you will reference
- Every behavior and architecture description
- For each, cite the source: the `jj` command output that produced it, or the file:line where you read it
...
3. **Recent activity** — not raw `jj log`. A human-readable narrative grouped by theme: feature work, bug fixes, refactors, infrastructure. Timeline visualization with the most significant changes called out. For each theme, a one-sentence summary of what happened and why it mattered.
4. **Decision log** — key design decisions from the time window. Extracted from change descriptions, conversation history, plan docs, progress docs. Each entry: what was decided, why, what was considered. This is the highest-value section for fighting cognitive debt — the reasoning that evaporates first.
5. **State of things** — *visual treatment: use the KPI card pattern from css-patterns.md — large hero numbers for working/broken/blocked/in-progress counts, with color-coded trend indicators.* A dashboard of:
   ...
   - What's in progress (working-copy changes, active bookmarks, active TODOs)
