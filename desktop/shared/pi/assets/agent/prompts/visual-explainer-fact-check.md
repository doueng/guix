...
---
Load the visual-explainer skill, then verify the factual accuracy of a document that makes claims about a codebase. Read the file, extract every verifiable claim, check each against the actual code and `jj` history, correct inaccuracies in place, and add a verification summary.
...
- **HTML review pages** (diff-review, plan-review, project-recap): detect from page content, verify against the `jj` revset or plan file the review was based on
...
- **Temporal**: `jj` history claims, change attributions, timeline entries
...
- For claims about `jj` history: re-run `jj` commands (`jj diff --stat`, `jj log`, `jj diff --summary`, etc.) and compare output against the document's numbers
- For diff-reviews: read both the ref version (`jj file show -r <ref> path/to/file`) and working-copy version to verify before/after claims aren't swapped or fabricated
- For plan docs: verify that files, functions, and types the plan references actually exist and behave as described
- For project-recaps: re-run `jj log` commands to verify activity narrative and timeline
