...
2. Start isolated work using the repository's native VCS: `jj new -m "autoresearch: <goal>"` in a Jujutsu repo, otherwise `git checkout -b autoresearch/<goal>-<date>`.
...
- **Resuming:** if `autoresearch.md` exists, read it plus the native VCS log (`jj log` or `git log`), then continue looping.
