...
interface AutoresearchRuntime {
  ...
  experimentSnapshot: Record<string, string> | null;
  experimentRoot: string | null;
  completedRun: boolean;
  turnBaselineClean: boolean;
  state: ExperimentState;
...
export const LogParams = Type.Object({
  commit: Type.String({ description: "VCS commit hash (short, 7 chars)" }),
  metric: Type.Number({
    description:
...
type VcsContext = { kind: VcsKind; root: string };
...
async function resolveVcsContext(pi: ExtensionAPI, workDir: string): Promise<VcsContext> {
  const execOpts = { cwd: workDir, timeout: 5_000 };
  try {
    const jjResult = await pi.exec("jj", ["root"], execOpts);
    if (jjResult.code === 0) return { kind: "jj", root: jjResult.stdout.trim() };
  } catch {
    // Try git below.
  }

  try {
    const gitResult = await pi.exec("git", ["rev-parse", "--show-toplevel"], execOpts);
    if (gitResult.code === 0) return { kind: "git", root: gitResult.stdout.trim() };
  } catch {
    // No supported VCS detected.
  }

  return { kind: "none", root: workDir };
}

async function shortHeadCommit(pi: ExtensionAPI, workDir: string, vcs: VcsKind): Promise<string | null> {
  try {
    if (vcs === "jj") {
      const result = await pi.exec(
        "jj",
        ["log", "-r", "@-", "--no-graph", "-T", "commit_id.short(7)"],
        { cwd: workDir, timeout: 5_000 }
      );
      const id = (result.stdout || "").trim();
      return result.code === 0 && id.length >= 7 ? id.slice(0, 7) : null;
    }
    if (vcs === "git") {
      const result = await pi.exec("git", ["rev-parse", "--short=7", "HEAD"], {
        cwd: workDir,
        timeout: 5_000,
      });
      const id = (result.stdout || "").trim();
    ...
    // Keep caller-provided commit id.
  }
  return null;
}

async function commitExperimentChanges(
  pi: ExtensionAPI,
  workDir: string,
  commitMsg: string,
  experimentPaths: string[]
): Promise<{ message: string; commit: string | null }> {
  if (experimentPaths.length === 0) {
    return { message: "📝 Nothing to commit (experiment changed no files)", commit: null };
  }
  const { kind: vcs, root } = await resolveVcsContext(pi, workDir);
  if (vcs === "none") return { message: "⚠️ VCS commit skipped: no jj or git repository detected", commit: null };

  if (vcs === "jj") {
    const execOpts = { cwd: root, timeout: VCS_TIMEOUT_MS };
    const commitResult = await pi.exec("jj", ["commit", "-m", commitMsg, "--", ...experimentPaths], execOpts);
    const output = combinedOutput(commitResult);
    if (commitResult.code !== 0) {
      throw new Error(`jj commit failed (exit ${commitResult.code}): ${output.slice(0, 200)}`);
    }

    const firstLine = output.split("\n")[0] || "committed working-copy change";
    return {
      message: `📝 JJ: committed — ${firstLine}`,
      commit: await shortHeadCommit(pi, root, vcs),
    };
  }

  const execOpts = { cwd: root, timeout: VCS_TIMEOUT_MS };
  const addResult = await pi.exec("git", ["add", "-A", "--", ...experimentPaths], execOpts);
  if (addResult.code !== 0) {
    const addErr = combinedOutput(addResult);
    throw new Error(`git add failed (exit ${addResult.code}): ${addErr.slice(0, 200)}`);
  }

  const stagedResult = await pi.exec("git", ["diff", "--cached", "--name-only", "--no-renames", "-z"], execOpts);
  if (stagedResult.code !== 0) throw new Error(`git staged diff failed: ${combinedOutput(stagedResult).slice(0, 200)}`);
  const stagedPaths = stagedResult.stdout.split("\0").filter(Boolean);
  if (stagedPaths.length === 0) return { message: "📝 Git: nothing to commit (working tree clean)", commit: null };
  if (!samePaths(stagedPaths, experimentPaths)) {
    throw new Error("git index contains changes outside the experiment paths");
  }

  const gitResult = await pi.exec("git", ["commit", "-m", commitMsg, "--", ...experimentPaths], execOpts);
  const gitOutput = combinedOutput(gitResult);
  if (gitResult.code !== 0) {
...
async function revertExperimentChanges(pi: ExtensionAPI, workDir: string, experimentPaths: string[]): Promise<string> {
  if (experimentPaths.length === 0) return "📝 No experiment changes to revert";
  const { kind: vcs, root } = await resolveVcsContext(pi, workDir);
  if (vcs === "jj") {
    const result = await pi.exec("jj", ["restore", "--", ...experimentPaths], {
      cwd: root,
      timeout: VCS_TIMEOUT_MS,
    });
    const output = combinedOutput(result);
    if (result.code !== 0) throw new Error(`jj restore failed (exit ${result.code}): ${output.slice(0, 200)}`);
    return "📝 JJ: reverted changes — autoresearch files preserved";
  }

  if (vcs === "git") {
    const execOpts = { cwd: root, timeout: VCS_TIMEOUT_MS };
    for (const filePath of experimentPaths) {
      const tracked = await pi.exec("git", ["ls-files", "--error-unmatch", "--", filePath], execOpts);
      if (tracked.code !== 0) continue;
      const restoreResult = await pi.exec("git", ["restore", "--staged", "--worktree", "--", filePath], execOpts);
      if (restoreResult.code !== 0) {
        throw new Error(`git restore failed for ${filePath}: ${combinedOutput(restoreResult).slice(0, 200)}`);
      }
    }
    const cleanResult = await pi.exec("git", ["clean", "-fd", "--", ...experimentPaths], execOpts);
    if (cleanResult.code !== 0) {
      throw new Error(`git clean failed (exit ${cleanResult.code}): ${combinedOutput(cleanResult).slice(0, 200)}`);
    }
    return "📝 Git: reverted changes — autoresearch files preserved";
  }

  return "⚠️ VCS revert skipped: no jj or git repository detected";
}

async function existingUserChanges(pi: ExtensionAPI, workDir: string): Promise<string[]> {
  const { kind: vcs, root } = await resolveVcsContext(pi, workDir);
  if (vcs === "none") return [];

  const result = vcs === "jj"
    ? await pi.exec("jj", ["diff", "-T", 'path ++ "\\0"'], { cwd: root, timeout: VCS_TIMEOUT_MS })
    : await pi.exec("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], { cwd: root, timeout: VCS_TIMEOUT_MS });
  if (result.code !== 0) {
    throw new Error(`${vcs} status failed (exit ${result.code}): ${combinedOutput(result).slice(0, 200)}`);
  }

  const changedPaths = vcs === "jj"
    ? result.stdout.split("\0")
    : parseGitStatusPaths(result.stdout);
  const statePath = path.resolve(autoresearchJsonlPath(workDir));
  return changedPaths
    .filter(Boolean)
    .filter((filePath) => path.resolve(root, filePath) !== statePath);
}
...
function snapshotPaths(paths: string[]): Record<string, string> {
  return Object.fromEntries(paths.map((filePath) => {
    try {
      ...
      const prefix = `${stat.mode}:${stat.size}:`;
      if (stat.isSymbolicLink()) return [filePath, `${prefix}link:${fs.readlinkSync(filePath)}`];
      if (stat.isFile()) {
        const hash = createHash("sha256").update(fs.readFileSync(filePath)).digest("hex");
        return [filePath, `${prefix}file:${hash}`];
      }
      if (stat.isDirectory() && fs.existsSync(path.join(filePath, ".git"))) {
        throw new Error(`Autoresearch does not support submodule changes: ${filePath}`);
      }
      return [filePath, `${prefix}other`];
    } catch (error) {
      const code = (error as NodeJS.ErrnoException).code;
      if (code === "ENOENT") return [filePath, "missing"];
      throw error;
    }
  }));
}
...
async function pendingRunSafetyError(
  pi: ExtensionAPI,
  workDir: string,
  runtime: AutoresearchRuntime
): Promise<string | null> {
  if (!runtime.completedRun || !runtime.experimentSnapshot || !runtime.experimentRoot) {
    return "No completed run is available to log. Run the experiment again from a clean turn.";
  }
  const currentRoot = (await resolveVcsContext(pi, workDir)).root;
  if (currentRoot !== runtime.experimentRoot) return "The experiment repository changed after the run completed.";
  const currentPaths = await existingUserChanges(pi, workDir);
  if (!samePaths(currentPaths, runtime.experimentPaths)) return "Files changed after the experiment completed.";
  const currentSnapshot = snapshotPaths(runtime.experimentPaths.map((filePath) => path.resolve(runtime.experimentRoot!, filePath)));
  if (JSON.stringify(currentSnapshot) !== JSON.stringify(runtime.experimentSnapshot)) {
    return "Experiment files changed after the run completed.";
  }
  return null;
}
...
function isAutoresearchShCommand(command: string): boolean {
  ...
  return normalized === "bash autoresearch.sh" || normalized === "./autoresearch.sh";
}
...
function clearPendingRun(runtime: AutoresearchRuntime): void {
  runtime.experimentPaths = [];
  runtime.experimentSnapshot = null;
  runtime.experimentRoot = null;
  runtime.completedRun = false;
}
...
export default function autoresearchExtension(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    const workDir = resolveWorkDir(ctx.cwd);
    try {
      runtime.turnBaselineClean = (await existingUserChanges(pi, workDir)).length === 0;
    } catch {
      runtime.turnBaselineClean = false;
    }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const workDir = resolveWorkDir(ctx.cwd);
      try {
        const changedPaths = await existingUserChanges(pi, workDir);
        if (changedPaths.length > 0) {
          return {
            content: [{
              type: "text",
              text: `❌ Autoresearch requires an isolated clean change because keep/discard operates on the whole current change. Preserve these edits first, then start a new empty JJ change or clean Git worktree:\n${changedPaths.map((filePath) => `- ${filePath}`).join("\n")}`,
            }],
            details: {},
          };
        }
        runtime.turnBaselineClean = true;
      } catch (e) {
        return {
          content: [{ type: "text", text: `❌ Could not verify worktree safety: ${e instanceof Error ? e.message : String(e)}` }],
          details: {},
        };
      }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (!runtime.turnBaselineClean) {
        return {
          content: [{
            type: "text",
            text: "❌ Autoresearch did not start this turn from a clean baseline. Preserve unrelated edits and start a new turn before running an experiment.",
          }],
          details: {},
        };
      }

      if (runtime.completedRun) {
        return {
          content: [{ type: "text", text: "❌ Log the completed experiment before starting another run." }],
          details: {},
        };
      }
      ...
      const hasAutoresearchScript = fs.existsSync(autoresearchShPath);
      if (hasAutoresearchScript && !isAutoresearchShCommand(params.command)) {
        return {
          content: [{
            type: "text",
      ...
      clearPendingRun(runtime);
      const vcsContext = await resolveVcsContext(pi, workDir);
      if (vcsContext.kind === "none") {
        return {
          content: [{ type: "text", text: "❌ Autoresearch requires a Jujutsu or Git repository for safe keep/discard handling." }],
          details: {},
        };
      }
      runtime.experimentRoot = vcsContext.root;
      runtime.experimentPaths = await existingUserChanges(pi, workDir);
      runtime.experimentSnapshot = snapshotPaths(runtime.experimentPaths.map((filePath) => path.resolve(runtime.experimentRoot!, filePath)));
      runtime.turnBaselineClean = false;
      ...
      const { exitCode, killed: timedOut, output, tempFilePath: streamTempFile, actualTotalBytes } = await new Promise<{
        ...
        const child = hasAutoresearchScript
          ? spawn("bash", [autoresearchShPath], {
              cwd: workDir,
              detached: true,
      ...
      const finalPaths = await existingUserChanges(pi, workDir);
      const finalSnapshot = snapshotPaths(runtime.experimentPaths.map((filePath) => path.resolve(runtime.experimentRoot!, filePath)));
      if (!samePaths(finalPaths, runtime.experimentPaths)
          || JSON.stringify(finalSnapshot) !== JSON.stringify(runtime.experimentSnapshot)) {
        throw new Error("The benchmark or checks modified the worktree. Preserve those changes manually and start a new clean turn.");
      }
      runtime.completedRun = true;
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "log_experiment automatically commits with jj in jj repos (git fallback elsewhere) on 'keep', and auto-reverts code changes on 'discard'/'crash'/'checks_failed' (autoresearch files are preserved). Do NOT commit or revert manually.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const safetyError = await pendingRunSafetyError(pi, workDir, runtime);
      if (safetyError) {
        return {
          content: [{ type: "text", text: `❌ ${safetyError} Preserve those edits and rerun before logging.` }],
          details: {},
        };
      }
      ...
      const experiment: ExperimentResult = {
        commit: params.commit.slice(0, 7).padEnd(7, "0"),
      ...
      const lateSafetyError = await pendingRunSafetyError(pi, workDir, runtime);
      if (lateSafetyError) {
        return {
          content: [{ type: "text", text: `❌ ${lateSafetyError} Preserve those edits and rerun before logging.` }],
          details: {},
        };
      }

      experiment.confidence = computeConfidence(
        [...state.results, experiment],
        state.currentSegment,
        state.bestDirection
      );
      ...
      const jsonlPath = autoresearchJsonlPath(workDir);
      const jsonlExisted = fs.existsSync(jsonlPath);
      const jsonlOriginalSize = jsonlExisted ? fs.statSync(jsonlPath).size : 0;
      const jsonlLine = JSON.stringify(jsonlEntry) + "\n";
      const commitValueOffset = jsonlOriginalSize
        + Buffer.byteLength(jsonlLine.slice(0, jsonlLine.indexOf('"commit":') + '"commit":'.length));
      ...
      try {
        ...
        try {
          if (jsonlExisted) fs.truncateSync(jsonlPath, jsonlOriginalSize);
          else if (fs.existsSync(jsonlPath)) fs.unlinkSync(jsonlPath);
        } catch (rollbackError) {
          return {
            content: [{
              type: "text",
              text: `❌ Failed to persist experiment and roll back the partial JSONL entry: ${e instanceof Error ? e.message : String(e)}; ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`,
            }],
            details: {},
          };
        }
        return {
          content: [{ type: "text", text: `❌ Failed to persist experiment before VCS action: ${e instanceof Error ? e.message : String(e)}` }],
          details: {},
        };
      }

      let vcsMessage: string;
      let vcsApplied = false;
      try {
        if (params.status === "keep") {
          ...
          const commitResult = await commitExperimentChanges(pi, workDir, commitMsg, runtime.experimentPaths);
          vcsApplied = true;
          vcsMessage = commitResult.message;
          if (commitResult.commit) {
            experiment.commit = commitResult.commit;
            jsonlEntry.commit = commitResult.commit;
            fs.writeSync(jsonlFd!, JSON.stringify(commitResult.commit), commitValueOffset, "utf8");
            fs.fsyncSync(jsonlFd!);
          }
        } else {
          vcsMessage = `${await revertExperimentChanges(pi, workDir, runtime.experimentPaths)} (${params.status})`;
          vcsApplied = true;
        }
        ...
        if (jsonlFd !== null) {
          try { fs.closeSync(jsonlFd); } catch {}
          jsonlFd = null;
        }
        if (vcsApplied) {
          vcsMessage = `⚠️ VCS action succeeded, but commit metadata finalization failed: ${e instanceof Error ? e.message : String(e)}`;
        } else {
        try {
          if (jsonlExisted) fs.truncateSync(jsonlPath, jsonlOriginalSize);
          else fs.unlinkSync(jsonlPath);
        } catch (rollbackError) {
          return {
            content: [{
              type: "text",
              text: `❌ VCS operation failed and JSONL rollback also failed: ${e instanceof Error ? e.message : String(e)}; ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`,
            }],
            details: {},
          };
        }
        return {
          content: [{ type: "text", text: `❌ VCS operation failed; experiment remains pending: ${e instanceof Error ? e.message : String(e)}` }],
          details: {},
        };
        }
      }
