...
Follow the visual-explainer skill workflow. Read the reference template, CSS patterns, and mermaid theming references before generating. Use a jj change-review aesthetic with red/green before/after panels, but vary fonts and palette from previous diagrams.

**Scope detection** — determine what to diff based on `$1`:
- Bookmark name (e.g. `main`, `trunk`): working copy vs that bookmark
- Change ID or revision ID: that specific change's diff (`jj show <rev>`)
- `@`: working-copy changes only (`jj diff`)
- Range (e.g. `main..@`, `abc123..def456`): diff between two revisions via `jj diff --from <left> --to <right>`
- Any other revset expression: use it directly if valid
- No argument: default to `main`

**Data gathering phase** — run these first to understand the full scope:
- `jj diff --stat --from <ref> --to @` for file-level overview
- `jj diff --summary --from <ref> --to @` for new/modified/deleted files (separate src from tests)
- Line counts: compare key files between `<ref>` and `@` (`jj file show -r <ref> path/to/file | wc -l` vs `jj file show -r @ path/to/file | wc -l`)
