...
export function buildTokenjuiceDetails(result: CompactResult, fullOutputPath?: string): TokenjuiceDetails {
  ...
  return {
    ...
    ...(typeof result.classification?.matchedReducer === "string"
      ...
      : {}),
    ...(result.rawRef?.id ? { rawArtifactId: result.rawRef.id } : {}),
    ...(result.rawRef?.path ? { rawArtifactPath: result.rawRef.path } : {}),
    ...(result.rawRef?.filteredTextPath ? { filteredTextPath: result.rawRef.filteredTextPath } : {}),
    ...(result.rawRef?.diffPath ? { diffPath: result.rawRef.diffPath } : {}),
