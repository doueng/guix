...
- Ghostty (the default terminal) and Kitty with Fish, JetBrains Mono and Catppuccin Mocha colors; Noctalia 5.1.0 as the Hyprland shell/launcher (with Wofi retained as a launcher fallback); `nmtui` network controls and the `bluetui` terminal Bluetooth manager (using the system BlueZ service).
